...
public class MemberServiceImpl implements MemberService{
	...
	public List<Member> getSearchMember(String searchKey, String searchValue) {
		...
		List<Member> memberList = memberMapper.getSearchMemberById(searchKey, searchValue);
		return memberList;
	}
	...
	public void removeMember(String memberGrade, String memberId) {
		switch (memberGrade) {
			case "mbr_grd_2" -> {
				// 1. 주문상세
				ordersMapper.removerOrderItemsBySellerId(memberId);
				// 2. 상품
			...
			case "mbr_grd_3" -> {
				List<String> ordersNumList = ordersMapper.getOrdersNumByCustomerId(memberId);
				if(ordersNumList != null && ordersNumList.size() > 0) {
					// 1. 주문상세
					ordersMapper.removeOrderItemByNum(ordersNumList);
					// 2. 주문
